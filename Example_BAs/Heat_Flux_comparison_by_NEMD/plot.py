...
BLOCK_VOLUME=(4865.022-286.740-(4865.022-4626.072))*9.5579996109999996*9.5579996109999996#
BLOCK_LENGTH = (4865.022-286.740-(4865.022-4626.072))#(4.779*100) #A
...
def lammps_heatflux(path):
    ...
    jpy = jpy *dt*1000/ BLOCK_VOLUME *CROSS_AREA * 1000 #in units of eV/ns=eVA/ps*(1/A**3)*A**2*1000ps/ns
...
for i in range(n_plot):
    ...
    m=0.6 #round(max([float(np.max(jp_mtp)),float(np.max(etol_mtp))])*1.1,2)
    ylim([0.0, m])
    plt.gca().set_yticks(linspace(0.0, m, 4))
    plt.xlabel('Time (ns)')
    plt.ylabel('Accumulated heat (keV)')
